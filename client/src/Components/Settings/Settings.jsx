 ...
 render() {
  return (
    <div className={style.block}>
      <h3 className={style.title}>Редактирование фотографий</h3>

      {
        this.props.fetching ? <div className={style.fetching}><Preloader /></div> : 
        <> 
          <span className={style.subtitle}>Аватарка</span>  
          <span className={style.rec}>Рекомендуемый размер: 300x300</span>
          <EditAvatarForm onSubmit={this.editAvatar} /> 

          <span className={style.subtitle}>Фон для профиля</span>   
          <span className={style.rec}>Рекомендуемый размер: 1230x820</span>
